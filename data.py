from typing import Iterator, List, Dict, Optional, Iterable, Callable
import random
import itertools
import torch
...
class StreamingPackedLMDataset(IterableDataset):
    ...
    def __init__(
        ...
        if not callable(texts_factory):
            raise ValueError("StreamingPackedLMDataset requires a callable that yields a fresh iterator per epoch")
    ...
    def __iter__(self):
        ...
        for idx, text in enumerate(texts_iter):
            ...
            if not isinstance(text, str) or len(text) == 0:
                continue
            enc = self.tokenizer(text, add_special_tokens=True, truncation=True, max_length=self.seq_len)
            ids = enc['input_ids']
            if self.add_eos and eos_id is not None and (len(ids) == 0 or ids[-1] != eos_id):
                ids = ids + [eos_id]
            buffer.extend(ids)
            docs_seen += 1
        ...
        # At the end, flush one partial if long enough to be useful (optional)
        # We skip to keep fixed-length batches consistent.


def load_text_dataset(name: str, config_name: str = None, split: str = "train", text_field: str = "text", streaming: bool = False, max_shards=None, verbose=True):
    """Load a HF dataset and yield raw text strings."""
    ds = load_dataset(name, config_name, split=split, streaming=streaming)
    if verbose:
        print(f"Loading dataset: {name}/{config_name}, split: {split}, streaming: {streaming}")
...
def build_token_sequences(tokenizer, texts: Iterator[str], max_docs: int = 20000, seq_len: int = 1024):
    ...
    for i, t in enumerate(texts):
        if i % 1000 == 0:
            print(f"Tokenizing document {i+1}/{max_docs if max_docs else 'inf'}", end='\r')
        enc = tokenizer(t, add_special_tokens=True, truncation=True, max_length=seq_len)
        ids = enc['input_ids']
...
def make_dataloaders(cfg, tokenizer):
    ...
    train_token_seqs = build_token_sequences(tokenizer, train_texts, max_docs=train_docs, seq_len=seq_len)
    ...
    val_token_seqs = build_token_sequences(tokenizer, val_texts, max_docs=val_docs, seq_len=seq_len)
